...
with open("data/reid_q_t.p", "rb") as f:
    question_tags_list = pickle.load(f)

with open("data/stack_tag_edge_0.65.p", "rb") as f:
    tag_parent_child_edges = pickle.load(f)

with open("data/reid_question.p", "rb") as f:
    final_valid_question = pickle.load(f)


with open("data/stack_glove_topic_emb.p", "rb") as f:
...
whole_num_questions = len(final_valid_question)
# print(whole_num_questions)
whole_num_tags = tag_name_embeddings.shape[0]
dim_size = tag_name_embeddings.shape[1]
question_embeddings = torch.rand(whole_num_questions,dim_size)
...
tag_name_embeddings = torch.tensor(tag_name_embeddings,dtype=torch.float32)
...
# model = HGTModel(question_embeddings,tag_name_embeddings,node_dict,edge_dict,100).to(device)
...
train_loader = DataLoader(dataset, batch_size=4000, shuffle=True, drop_last=True)
...
for epoch in range(1, 1001):
    ...
    if epoch % 100 == 0:
        ...
        if epoch % 1000 == 0:
            ...
            with open('0.5_stack_mine.csv', 'a+', encoding='UTF8', newline='') as f:
                writer = csv.writer(f)
