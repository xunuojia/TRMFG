...
from layers import MineModel,RGCNModel,HGTModel,HERE_Last
import torch
...
from evaluations import get_final_eval, evaluate_mean_global_metrics,get_final_vision
import warnings
...
train_question_tag_edges, train_q_t_dict, test_q_t_dict,train_index, test_index= pretreatment_second(question_tags_list,question_embeddings,tag_name_embeddings)
...
model = HERE_Last(question_embeddings,tag_name_embeddings,768,768).to(device)
...
def mine_info_bpr_loss(a_embeddings, b_embeddings, pos_edges):
    ...
    pos_logits,_ = torch.max(pos_num_heads,dim=-1)#4000,4,1
    neg_logits,_ = torch.max(neg_num_heads,dim=-1)
    
    # pos_logits = torch.mean(pos_num_heads,dim=-1)
    # neg_logits = torch.mean(neg_num_heads,dim=-1)
    # print(pos_logits.shape)

    pos_logits = pos_logits.squeeze(1)#4000,4
...
for epoch in range(1, 1001):
    ...
    if epoch % 1000 == 0:
        model.eval()
        question_h, tag_emb = model()
        # question_h, tag_emb = model(question_embeddings,tag_node, eindex)
        # tag_h = tag_emb[test_index]
        tag_h = tag_emb
        record = get_final_vision(test_q_t_dict, train_q_t_dict,
                                                            question_h.detach().cpu().numpy(),
                                                            tag_h.detach().cpu().numpy(),
                                                            k_list=[5], metrics=["precision", "recall", "ndcg"])
        print(record)
        with open("data/try_here.p", "wb") as f:
    ...
    #         with open('0.25_here_try_act.csv', 'a+', encoding='UTF8', newline='') as f:
    #             writer = csv.writer(f)
